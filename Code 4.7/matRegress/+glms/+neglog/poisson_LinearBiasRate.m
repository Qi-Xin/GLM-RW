...
switch nargout
    case 1
        f = fnlin(xproj);
        bias=1.82*10*1e-6*(1:1:1000);
        f=f+[bias,bias,bias,bias,bias,bias,bias,bias,bias,bias]';
	nzidx = f ~= 0;
	if any(y(~nzidx) ~= 0)
	...
	if any(y(~nzidx) ~= 0)
	    ...
	    L = -y(nzidx)'*log(f(nzidx)) + sum(f) + 5e-4 * wts' * wts ; % neg log-likelihood
	end
