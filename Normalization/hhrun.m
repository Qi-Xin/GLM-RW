function [spikeNum,V] = hhrun(I,tspan, v, mi, hi, ni,Plot)   
...
%   Example 1 -
%   
%   hhrun(0.1, 500, -65, 0.5, 0.06, 0.5, 0)
%   where,
...
%   plot(t,V) will generate the time series plot
  ...
  for i=1:loop-1 
      V(i+1) = V(i) + dt*(gNa*m(i)^3*h(i)*(eNa-(V(i)+65)) + gK*n(i)^4*(eK-(V(i)+65)) + gL*(eL-(V(i)+65)) + I(ceil(i/1e3)));
