...
repnum = 3e3;
RefractoryTimeList = [0,1,3,5,10,20];
repeat = 3;
db_rawV = NaN*zeros(length(RefractoryTimeList),repeat);
db_rawV2 = NaN*zeros(length(RefractoryTimeList),repeat);
db_rawL1 = NaN*zeros(length(RefractoryTimeList),repeat);
db_rawL2 = NaN*zeros(length(RefractoryTimeList),repeat);
db_baseline_raw = NaN*zeros(length(RefractoryTimeList),repeat);
numerator_raw = NaN*zeros(length(RefractoryTimeList),repeat);
denominator_raw = NaN*zeros(length(RefractoryTimeList),repeat);

maxSig = 6e-2;  % 7e-2, 12e-2, 4e-2
minSig = 2e-2;
signalType = 3; % 1 for no signal, 2 for square wave, 3 for gamma, 4 for Gaussian random
I_per = zeros(1,1e3);
if signalType == 2
...
for ii = 1:length(RefractoryTimeList)
    for jj = 1:repeat
        dt = 1e-3;
        RefractoryTime = RefractoryTimeList(ii);
        ...
        if RefractoryTime ~= 0
            for mm = 1:repnum
                for nn = 1:1e3
                    if ST(mm,nn)==1
                        ST(mm,(nn+1):(nn+RefractoryTime)) = zeros(1,RefractoryTime);
                    end
                end
            end
        end
        ...
        db_true = 10*log10(SNR_direct);
        SNR_indirect = var(mean(ST)) / mean(var(ST));
        SNR_indirect2 = var(mean(ST)) / mean(mean(ST));
        numerator_raw(ii,jj) = var(mean(ST));
        denominator_raw(ii,jj) = mean(mean(ST));
        SNR_indirect_baseline = var(mean(ST(:,201:800))) / mean(var(ST(:,[1:200,801:1000])));
        db_V = 10*log10(SNR_indirect);
        db_rawV(ii,jj) = db_V;
        db_rawV2(ii,jj) = 10*log10(SNR_indirect2);
...
figure;
hold on
errorbar(RefractoryTimeList,mean(db_rawV,2),std(db_rawV')');
errorbar(RefractoryTimeList,mean(db_baseline_raw,2),std(db_baseline_raw')');
%errorbar(RefractoryTimeList,mean(db_rawL1,2),std(db_rawL1')');
%errorbar(RefractoryTimeList,mean(db_rawL2,2),std(db_rawL2')');
plot([0,20],[db_true,db_true]);

xlabel('Length of Absolute Refractory Period');
ylabel('SNR /db');
title('SNR');
legend('Variance-based SNR','Variance-based SNR',...
    'Likelihood-based SNR w/H','Likelihood-based SNR w/out H','Ground True SNR');
xlim([0 20])


figure;
hold on
norNumerator = mean(numerator_raw,2);
norNumerator = normalizeNumerator(1);
norDenominator = mean(denominator_raw,2);
norDenominator = normalizeDenominator(1);
errorbar(RefractoryTimeList,mean(numerator_raw,2)/norNumerator,std(numerator_raw')'/norNumerator);
errorbar(RefractoryTimeList,mean(denominator_raw,2)/norDenominator,std(denominator_raw')'/norDenominator);
legend('Numerator','Denominator');

% compare var(mean(ST))/mean(var(ST)) and var(mean(ST))/mean(mean(ST))
%{
figure;
hold on
errorbar(RefractoryTimeList,mean(db_rawV,2),std(db_rawV')');
errorbar(RefractoryTimeList,mean(db_rawV2,2),std(db_rawV2')');
%}