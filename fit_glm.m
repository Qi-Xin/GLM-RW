...
if plotFlag
    ...
    plot((1:length(k)),k);
    if fit_k == 1
        fill([(1:length(k)) fliplr(1:length(k))],[kL' fliplr(kU')],'b','facealpha',0.2,'edgealpha',0,'HandleVisibility','off');
    end
    xlim([0 length(k) ]);
    set(gca,'xtick',0:round(length(k)/2):length(k),'xticklabel',round(fliplr(-dt*(0:round(length(k)/2):length(k)))))
    xlabel('time (ms)')
    ...
    xlim([1 length(h) ]);
    %set(gca,'xtick',0:round(length(h)/5):length(h),'xticklabel',round(dt*(0:round(length(h)/5):length(h))))
    fill([(1:length(h)) fliplr(1:length(h))],[hL' fliplr(hU')],'b','facealpha',0.2,'edgealpha',0,'HandleVisibility','off');
    xlabel('time (ms)')
