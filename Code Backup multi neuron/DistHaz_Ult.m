...
%% Simulation
[ISI,spike_timing,y_sparse,V,inputE,inputI] = GetISI(tau_E,tau_I,tau_M,V_E,V_I,p,q,V_th,V_reset,tot_N,tot_t,dt,Nn);
ISI = 0.1*ISI;
...
%}
subplot(6,1,4);
plot(1e-4*(1:T/1),inputE(1,1:T/1)-inputI(1,1:T/1));
xlabel('t/s');
ylabel('Amplitude');
title('Equivalent Poisson Input');
subplot(6,1,5);
plot(1e-4*(1:T),V(1,1:T));
xlabel('t/s');
ylabel('Normalized Voltage');
title('Voltage');
subplot(6,1,6);
plotraster(reshape(y_sparse(1,1:1*T),[],1)',1:T,'Spike train');
