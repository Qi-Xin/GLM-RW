...
for hnum = 1:nhbasis
    yconvhi(:,hnum) = sameconv_OnlyLastOne(y,hbasis(:,hnum));
end
...
if fit_k == 1
    ...
    %[prs,dev,stats] = glmfit([yconvhi],y,'poisson','constant','off');
    [prs,dev,stats] = glmfit([yconvhi],y,'poisson','constant','on');
end
se = stats.se;
prsL = prs-se;
prsU= prs+se;
...
if plotFlag
    ...
    xlabel('time (ms)')
    title('log hazard function')
    grid on
end

figure;hold on
plot((1:length(h)),exp(h),'b');
xlim([1 length(h) ]);
%set(gca,'xtick',0:round(length(h)/5):length(h),'xticklabel',round(dt*(0:round(length(h)/5):length(h))))
fill([(1:length(h)) fliplr(1:length(h))],exp([hL' fliplr(hU')]),'b','facealpha',0.2,'edgealpha',0,'HandleVisibility','off');
xlabel('time (ms)')
